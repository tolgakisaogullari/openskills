...
| `ingest-code-to-qdrant.py` | Ingest source code files | AUTO on pull: re-embeds just the changed files (incremental, background, post-merge/post-checkout `sumela_code_sync`); first build / empty collection promotes to a full walk. Force a full re-embed with `SUMELA_PULL_CODE_REINGEST=1`; pass `--changed-file <list>` for a manual incremental run. `--heal` re-ingests entries the index is missing or only partially holds — the hook runs it automatically every 6h; run it by hand if you suspect the index is incomplete |
